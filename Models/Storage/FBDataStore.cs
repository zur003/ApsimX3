namespace Models.Storage
{
    ...
    public class DataStore : Model, IStorageReader, IStorageWriter, IDisposable
    {
                                 ...
                                 string orderBy = null)
        {
            ...
            // This should be re-factored for Firebird, which can be slow doing SELECT queries with ORDER BY. Using a derived table should help.
            // See https://ib-aid.com/en/articles/45-ways-to-speed-up-firebird-database/
        ...
        public void AddUnitsForTable(string tableName, List<string> columnNames, List<string> columnUnits)
        {
            ...
            if (!connection.GetColumnNames("_Units").Contains("ColumnNumber"))
                connection.AddColumn("_Units", "ColumnNumber", "INTEGER");

            List<object[]> values = new List<object[]>();
            for (int i = 0; i < columnNames.Count; i++)
            {
                values.Add(new object[] { tableName, columnNames[i], columnUnits[i], i });
            }
            List<string> unitsColumns = new List<string>(new string[] { "TableName", "ColumnHeading", "Units", "ColumnNumber"});
            ...
            if (useFirebird)
            {
                ...
                foreach (Table table in dataToWrite)
                {
                    ...
                    foreach (Table.Column column in table.Columns)
                    {
                        ...
                        //{
                            insertSql.Append("INSERT INTO [_Units] ([TableName], [ColumnHeading], [Units], [ColumnNumber]) VALUES ('");
                            ...
                            insertSql.Append("','");
                            insertSql.Append(table.ColumnNumber(column.Name));
                            insertSql.Append("\');\n");
                        //}
                    }
                }
        ...
        public bool Open(bool readOnly)
        {
            ...
            if (!useFirebird)
            {
                ...
                if (!connection.TableExists("_Units"))
                    connection.ExecuteNonQuery("CREATE TABLE _Units (TableName TEXT, ColumnHeading TEXT, Units TEXT, ColumnNumber INTEGER)");
            }
            else
            {
                if (!connection.TableExists("_Checkpoints"))
                ...
                if (!connection.TableExists("_Units"))
                    connection.ExecuteNonQuery("CREATE TABLE \"_Units\" (\"TableName\" VARCHAR(50), \"ColumnHeading\" VARCHAR(50), \"Units\" VARCHAR(50), \"ColumnNumber\" INTEGER)");
            }
            ...
            foreach (Table table in tables)
            {
                if (!table.Name.StartsWith("_"))
                {
                    ...
                    foreach (Table.Column column in table.Columns)
                    {
                        ...
                        if (useFirebird)
                        {
                            string bracketedColumnName = "[" + column.Name.Substring(0, Math.Min(31, column.Name.Length)) + "]";
                            data = connection.ExecuteQuery("SELECT FIRST 1 " + bracketedColumnName + " FROM [" + table.Name + "] WHERE " + bracketedColumnName + " IS NOT NULL");
                        }
                        else
                        {
                            string bracketedColumnName = "[" + column.Name + "]";
                            data = connection.ExecuteQuery("SELECT " + bracketedColumnName + " FROM [" + table.Name + "] WHERE " + bracketedColumnName + " IS NOT NULL LIMIT 1");
                        }
