...
namespace UserInterface.Views
{
    ...
    public class PropertyTreeView : ViewBase, IPropertyTreeView
    {
        ...
        public System.Drawing.Image GetScreenshotOfRightHandPanel()
        {
            ...
            panelWindow.GetGeometry(out x, out y, out width, out height);
            Gdk.Pixbuf screenshot = Gdk.PixbufHelper.GetFromWindow(panelWindow, 0, 0, width, height); /////
            ...
            if (e.Event.Button == 1 && e.Event.Type == Gdk.EventType.ButtonPress)
            {
                ...
                if (treeview1.GetPathAtPos((int)e.Event.X, (int)e.Event.Y, out path, out col))
                {
                    ...
                    if (selPath != null && path.Compare(selPath) == 0)
                    {
                        ...
                        {
                            timer.Interval = 260; ///// treeview1.Settings.DoubleClickTime + 10;  // We want this to be a bit longer than the double-click interval, which is normally 250 milliseconds
