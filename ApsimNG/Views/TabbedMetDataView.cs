...
namespace UserInterface.Views
{
    ...
    public class TabbedMetDataView : ViewBase, IMetDataView
    {
        ...
        private void TabControl1_SelectedIndexChanged(object sender, SwitchPageArgs e)
        {
            ...
            {
                bool moved = false;
                Container parent = hboxOptions.Parent as Container;
                if (parent != null)
                {
                    switch (e.PageNum)
                    {
                        case 2:
                            if (parent != alignRainChart)
                            {
                                parent.Remove(hboxOptions);
                                alignRainChart.Add(hboxOptions);
                                moved = true;
                            }
                            break;
                        case 3:
                            if (parent != alignRainMonthly)
                            {
                                parent.Remove(hboxOptions);
