        ...
        public void EndEdit()
        {
            if (IsUserEditingCell)
            {
                ...
                else if (editControl is ComboBox)
                {
                    ...
                    text = (editControl as ComboBoxText).ActiveText;
        ...
        public Label GetColumnHeaderLabel(int colNo, Gtk.TreeView view = null)
        {
            ...
            foreach (Widget widget in view.AllChildren)
            {
                ...
                else if (i++ == colNo)
                {
                    ...
                    {
                        if (child.GetType() != typeof(Gtk.Box))
                            continue;
                        foreach (Widget grandChild in ((Gtk.Box)child).AllChildren)
                        {
        ...
        private void PopulateGrid()
        {
            ...
            for (int i = 0; i < numCols; i++)
            {
                ...
                if (!colAttributes.TryGetValue(i, out _))
                {
                    ...
                    attrib.ForegroundColor = new Gdk.Color(0, 0, 0); /////
                    attrib.BackgroundColor = new Gdk.Color(255, 255, 255); /////
        ...
        private void ComboRenderEditing(object sender, EditingStartedArgs e)
        {
            try
            {
                ...
                (e.Editable as ComboBox).Changed += (o, _) =>
                {
                    ...
                    if (currentCell != null && (o as ComboBoxText).ActiveText != null)
                        UpdateCellText(currentCell, (o as ComboBoxText).ActiveText);
                    EndEdit();
                };
            }
            catch (Exception err)
